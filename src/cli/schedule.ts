...
import * as out from '../lib/out'
import Student = require('../student')

// @ts-ignore: TS7016: Could not find a declaration file for module 'run-simple/lib/argv-parser'.
import ArgvParser = require('run-simple/lib/argv-parser')

import SpreadsheetRevisionQueue = require('../lib/spreadsheet-revision-queue')
import * as scheduler from '../scheduler'

function computeKeyFromEmailAddress(emailAddress: string) {
  return emailAddress.replace(/^([^@]+?)@dtechhs\.org$/, '$1')
}

export = function run(options: {[k: string]: any} = {argv: process.argv.slice(2)}) {
  ...
  function usage(code?: number, message?: string) {
  ...
  const argv = new ArgvParser(options.argv, {
    ...
    validate: function validate(key: string, value: string) {
      if (!this.aliases[key]) {
  ...
  const commandsByName: {[key: string]: () => void} = {
    '': merge,
    divvy,
    merge,
    remix
  ...
  function divvy() {
    ...
    emailAddresses.forEach((emailAddress) => {
      ...
      input.sheets.forEach((sheet: any) => {
  ...
  function remix() {
    ...
    for (let i = 0; i < 10000 && min > target; ++i) {
  ...
  function writeMasterScheduleUpdates(result: any) {
    ...
    const spreadsheetId = '1PzN9Zsh7QJzhOu3wz73_D-7eBIACCvhq1eexBeUPLW8'

    const scheduleByStudent = {} as any
    const studentsByEmailAddress = {} as any

    schedule.forEach(({student, subject, teacher}: scheduler.Need) => {
    ...
    Object.keys(scheduleByStudent).forEach((emailAddress: string) => {
    ...
    const unschedulable: {[emailAddress: string]: any} = {}

    result.unschedulable
      .sort((a: any, b: any) => a.student.emailAddress.localeCompare(b.student.emailAddress))
      .forEach(({subject, teacher, student}: any) => {
  ...
  function writeScheduleUpdates(scheduled: scheduler.Need[]) {
    const schedule = {} as any

    scheduled.forEach(({subject, teacher, student}: scheduler.Need) => {
    ...
    Object.keys(schedule).forEach((key: string) => {
      ...
      titles.forEach((title, index) => {
        ...
        const rows = students.map((student: Student) => {
          const course = student.courses.find((x: Student.Course) => `${x.bell}: ${x.subject}` === title)
          const pace = course && course.pace || 'On'
